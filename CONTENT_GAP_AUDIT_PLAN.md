...
| 8 | Only 8.22 flagged (`phantom_disregard`); already fixed in an earlier session (confirmed genuine, no further action needed) | Full page-by-page sweep, all 21 pages (`page_0264.png`–`page_0285.png` for the chapter body plus its closing colophon), every verse 8.1–8.28 checked directly against the page images; whole-chapter regex sweeps run after fixing for all previously-documented recurring patterns (em-dash fabrication marker, ಪಕ್ವ→ಪಕ್ಚ, ಕರ್ತೃತ್ವ→ಕರ್ತ್ಯತ್ವ, ಎಲ್ಲೆಡೆ→ಎಲ್ದೆಡೆ, avagraha-as-`$`, decoded `\uXXXX` escapes) — zero hits on all six | Confirmed the missing-chapter-opening-preamble gap class (E8) recurs a sixth time: a ~350-word essay (page 264, before 8.1's own śloka) on the six things Krishna promised to explain at the end of ch.7 (adhibhūta/adhyātma/adhidaiva/adhiyajña/brahma/karma) was absent from all four language files. A new **E9 cross-verse-leak instance** at the **8.21→8.22 boundary**: a stray duplicate fragment of 8.22's own śloka opening, with an independent corruption (`ಲಭ್ಯಸ್ತ್ರ` for `ಲಭ್ಯಸ್ತ್ವ`), had been appended to the tail of 8.21 — removed, since 8.22 already held the correct, complete version of the same line. Two genuinely corrupted words with outsized meaning: **8.8**'s `ಅಹೋರಣೀಯವಾಗಿ`, which should read `ಅಣೋರಣೀಯವಾಗಿ` (the Upaniṣadic "smaller than the smallest," aṇor aṇīyān — a single character substitution had turned a well-known phrase into a non-word); and **8.9**'s padaccheda gloss `ಸ ,ಲಿಸುವವನು`, a badly garbled fragment restored to `ಸ್ಮರಿಸುವವನು` ("one who remembers"). A **recurring ಎಲ್ಲಕ್ಕೆಂತ→ಎಲ್ಲಕ್ಕಿಂತ** ("than everything") vowel-confusion typo hit four separate times in this chapter alone (8.9, 8.20, and twice in 8.28) — the same pattern also has an untouched duplicate at 15.15, flagged for that chapter's future audit. Two other duplicate-passage typos were found and fixed only within this chapter's scope, each with an untouched duplicate elsewhere flagged for later: `ಇಚ್ಚೆಯಿಂದ`→`ಇಚ್ಛೆಯಿಂದ` at 8.8 (duplicate at 10.13) and `ಪ್ರಾಹಾಯಾಮ`→`ಪ್ರಾಣಾಯಾಮ` at 8.10 (duplicate at 4.29). Beyond these, roughly 25 further character/spacing-level KN-only fixes spread across nearly every verse in the chapter (dropped/swapped conjuncts, stray spaces, stray/missing punctuation, one stray anusvara, one inserted syllable in a śloka word `ಧಾರಹಣಾಮ್`→`ಧಾರಣಾಮ್`) — see commit message for the full itemized list. Two fixes were attempted, then **reverted** after closer high-resolution re-verification showed the book's own padaccheda line — not just the compound śloka — consistently used the "non-standard" spelling (`ಅಪ್ಪುವಂತಿ` at 8.15, `ತದಾ` at 8.25): a reminder that the padaccheda line, not the compressed śloka, is the ground truth for what belongs in this data file. Several other plausible-looking spellings were checked and confirmed as genuine page artifacts, not bugs, so they aren't re-flagged: `ದೃಡೀಕರಿಸಿ`, `ಕೊಡತಕ್ಕವ`, `ತನ್ತಮೇವೈತಿ` (an anusvara-to-nasal sandhi form), `ಪ್ರರಾಬ್ಧ` (matches the same book-spelling already documented from ch.7), `ಧರ್ಮವ್ಯಾದ`, `ಅವನನ್ನೇನ್ನೆ`, and `ಧೀರ್ಥವಾದ` (the book's own non-standard spelling for "lengthy"). Chapter closes cleanly with its colophon (`ಇತಿ ಅಷ್ಟಮೋಽಧ್ಯಾಯಃ`) on `page_0285.png`, verified against the data. | Preamble transcribed from `page_0264.png` and prepended to the `"8.1"` key in all four language files (KN transcribed from the page; EN translated closely; DEV composed as condensed independent Sanskrit prose per house style; HI translated closely, replacing what had been a bare chapter-title header with no essay at all). ~30 KN-only typo/gap fixes applied directly in `bannanje_kn.js` with assertion-guarded search-and-replace, including the 8.21/8.22 cross-verse-leak cleanup. All four files validated to parse (701 keys each), bundle rebuilt via `build-bundle.py`. | 2026-09-04 |
| 9 | No flagged items checked separately (audit went straight to full page-by-page verse read) | Full page-by-page sweep, all verses 9.1–9.34 checked directly against the page images (`page_0285.png`–`page_0318.png`); whole-chapter regex sweep run after fixing for all previously-documented recurring patterns (avagraha-as-`$`, ಎಲ್ದೆಡೆ→ಎಲ್ಲೆಡೆ, ಕರ್ತ್ಯತ್ವ, ಪಕ್ಚ, ನಿಷ್ಕ್ರೀಯ, stray double-periods/semicolons/spaces) — zero further hits beyond what page-reading had already found; a few ambiguous double-period hits inspected turned out to be intentional ellipsis markers in Vedic-quote citations (9.16/9.17), not corruption | Confirmed the missing-chapter-opening-preamble gap class (E8) recurs a seventh time: a ~350-word essay (page 285, before 9.1's own śloka) on the jñāna/vijñāna distinction and why Krishna revisits topics already covered in ch.7 was absent. One instance of the documented avagraha-misread-as-`$` pattern at 9.1. Beyond these, ~30 ordinary character/spacing-level KN-only fixes spread across the chapter: dropped/swapped conjuncts (ವರ್ಮ್ಮನಿ→ವರ್ತ್ಮನಿ at 9.3, ನಿಬಧ್ದನ್ತಿ→ನಿಬಧ್ನನ್ತಿ at 9.9, ಪ್ರತಾಃ→ವ್ರತಾಃ at 9.14, ಕುರುಷ್ಟ→ಕುರುಷ್ವ at 9.27 ×2, ಭಕ್ಕ್ಯಾ→ಭಕ್ತ್ಯಾ and ಭಕ್ಕುಪಹೃತಮಶ್ನಾಮಿ→ಭಕ್ತ್ಯುಪಹೃತಮಶ್ನಾಮಿ at 9.26); one meaningful semantic OCR corruption, ಹತ್ಯಕ್ಕೆ (murder) → ಸತ್ಯಕ್ಕೆ (truth) at 9.12; a missing English gloss restored, `(Attachment)` at 9.5, plus a gloss-word correction `(account)`→`(Portfolio)` at 9.16; a badly garbled phrase fixed at 9.31 (ಮಾಡಬಾರಡದ್ದನ್ನೆ ಲ್ಡಾ→ಮಾಡಬಾರದ್ದನ್ನೆಲ್ಲಾ); a missing paren + wrong vowel restored at 9.11 (ಶ್ರೀಲಕ್ಷ್ಮೀಭೂತಮಹೇಶಳು)→ಶ್ರೀಲಕ್ಷ್ಮಿ(ಭೂತಮಹೇಶಳು)); a vowel-length fix at 9.34 (ನನಗೆ→ನನಗೇ, twice); many stray periods/semicolons/spaces throughout (9.1, 9.3, 9.5, 9.9, 9.11, 9.12, 9.15, 9.16, 9.18, 9.20, 9.21, 9.26, 9.28, 9.32). One self-corrected error worth noting for future sessions: an initial "fix" to 9.17's Sanskrit vowel-alphabet enumeration (changing ಌ/ೡ to ಎ/ಏ/ಒ/ಓ based on a blurry glyph read) was caught and reverted after cross-referencing the verse's own correlated 51-divine-names list, whose positional structure proved the original ಌ/ೡ reading was correct — a reminder to cross-check ambiguous glyphs against correlated content on the same page before trusting a single read. Several plausible-looking spellings were checked and confirmed as genuine page/book artifacts, not bugs, so they aren't re-flagged: ಅಶ್ರದ್ದಧಾನಾಃ (9.3, book's own ದ್ದ spelling), ನಚ combined (9.5, intentional — explicitly discussed as a fused sandhi term), ಪಾರ್ಥಾ/ಪ್ರಥೆಯ ಮಗನೇ (9.13, the latter a deliberate etymological wordplay on the ಪ್ರಥ root), ದೃಡ vs ದೃಢ inconsistency (9.14, book's own spelling), ಪ್ರರಾಬ್ಧ (9.30, matches ch.7/ch.8-documented book-spelling), ಬೀಷ್ಮ vs ಭೀಷ್ಮ inconsistency (9.31, book's own spelling), ಆಸಾದ್ಯ/ಆಸಾಧ್ಯ conflicting instances within 9.20 (left matching the grammatically-correct compressed-śloka instance), ಇದಮ್ ನ ಮಮಃ (9.27, non-standard but matches page), ಭಗವದಃ/ವಾಸುದೇವಸ್ಯಃ visarga forms in a quoted verse at 9.28 (matches page exactly). | Preamble transcribed from `page_0285.png`/`page_0286.png` (spanning into page 287) and prepended to the `"9.1"` key in all four language files (KN transcribed from the page; EN translated closely; DEV composed as condensed independent Sanskrit prose per house style; HI close to KN structure) — completed 2026-09-05 after an initial session left EN/DEV/HI untouched, which was correctly flagged as an incomplete chapter. ~30 KN-only typo/gap fixes applied directly in `bannanje_kn.js` with assertion-guarded search-and-replace. All four files validated to parse (701 keys each), bundle rebuilt via `build-bundle.py`. | 2026-09-05 |
| 10 | Not started | Not started | — | — | — |

**Note (2026-09-04, reconciling a contradictory partial-session finding):**
a same-day-earlier session (commit `967af6d`) had started chapter 8's
sweep, reached only pages 265-266 (verses 8.1-8.3), and concluded "no
chapter-opening preamble gap: page 265 goes straight into 8.1's own
śloka." That conclusion was based on checking page 265 only — the
preamble essay this session found and restored sits on **page 264**,
one page earlier, immediately before the book's own "ಅಧ್ಯಾಯ ಎಂಟು"
heading. This is a reminder that the E8 preamble (when present) always
sits on the page *before* the numbered page a verse's own śloka starts
on, not on that same page — worth checking explicitly rather than
inferring from the first verse's own page alone. The two sessions'
independent fixes to 8.1-8.3 (this session's fuller sweep vs. the
partial session's 5 small fixes) were reconciled during a rebase with
no loss on either side — the partial session's 8.1 fix (stray periods,
a mismatched quote pair) had not been touched by this session's own
work on that verse, so both sets of fixes now coexist correctly.
