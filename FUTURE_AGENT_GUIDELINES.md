...
* **CORRECTION, 2026-09-03 — the fallback fix above did NOT actually work, and here's why.**
  Vinayak reported the rendering bug was still visible after the fallback fix shipped. Investigation (rendering the exact string through both fonts via HarfBuzz + Pillow/raqm, and checking each font's `cmap` table with fontTools) showed the real mechanism: **CSS `font-family` fallback only triggers when the primary font is missing a glyph for a character (`cmap` coverage), not when the primary font's internal shaping/GSUB rules mis-render a valid character sequence.** Both `Tiro Kannada` and `Noto Sans Kannada` have full `cmap` coverage for every individual character in `ಜ್ಜ್ಞಾ` (ja+virama+ja+virama+ña+vowel — a rare **3-consonant stack** produced by sandhi). Since `Tiro Kannada` never reports a missing glyph, the browser has no reason to fall through to `Noto Sans Kannada` — it renders the whole string in Tiro Kannada regardless of list order, and Tiro Kannada's GSUB rules simply don't know how to stack 3 consonants, producing the broken/detached output (visually confirmed by rendering both fonts to PNG: Tiro Kannada produces a garbled cluster with a stray mark; Noto Sans Kannada produces a clean compact conjunct).
  **Actual fix**: reordered every `body.lang-kn` font-family rule (and the canvas-map `fontFam` JS variable) so **`'Noto Sans Kannada'` is primary**, with `'Tiro Kannada'` kept only as a secondary fallback. This is the only reliable fix for a font-internal shaping bug — fallback order doesn't help when the buggy font "supports" every character.
  **Devanagari checked and found NOT affected**: the same sandhi cluster rendered correctly in both `Tiro Devanagari Sanskrit` and `Noto Sans Devanagari` (verified the same way), so no equivalent change was needed for `lang-dev`/`lang-hi`.
  **Lesson for future agents**: when a rendering bug survives a "add a fallback font" fix, suspect a font-internal shaping bug rather than a missing-glyph problem — `cmap` coverage and OpenType shaping correctness are different things, and only the former drives CSS fallback. Verify by actually rendering the exact string (not just eyeballing the font list) through the candidate fonts — `fontTools` for `cmap` checks, HarfBuzz (`hb-shape`) or Pillow-with-raqm for real shaped output — before declaring a font-related fix complete.
...
* **Resolved 2026-09-03**: Vinayak confirmed there's no intended real font behind the name — dropped `'Tiro Devanagari Hindi'` entirely and set `lang-hi` to use `'Noto Sans Devanagari'` directly as the primary font. Fixed in all 5 locations: the 4 CSS rules in `viewer-src.html` (`ul.entry-list li .label`, `.focus-card h1`, `.relations li .other`, `.map-empty-title`) plus the canvas-map `fontFam` JS variable. Also found and fixed a 6th occurrence in `build-decks.js`'s `FONTS.hi` config (a separate, non-archived build script for slide decks) that had the identical dead reference. Historical snapshots under `archive/` (`viewer_online_snapshot_2026.html`, `index_snapshot_2026.html`) still contain the old reference — left untouched intentionally, as they're point-in-time archives, not live code.
