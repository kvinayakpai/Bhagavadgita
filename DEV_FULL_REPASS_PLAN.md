...
| 2 | 2.1–2.72 | 🟡 **IN PROGRESS 2026-09-05** | Ratio-scanned all 72 verses against KN (length ratio DEV/KN). Unlike ch.1, chapter 2 shows real, substantial condensation in a cluster of verses — the philosophically dense back half (2.44–2.61) and a few early verses (2.11, 2.14–2.16, 2.39) sit at 0.15–0.45 ratio, versus ch.1's typical 0.5–0.75. Two verses fixed this session: **2.44** (missing the 20-deity tattvābhimāni-devatā enumeration behind Gaṇapati's "21st tattva" status — restored), **2.45** (missing the yoga/kṣema definitions — "yoga is gaining the unhad, kṣema is keeping the had" — restored). 2.46/2.47 spot-checked and found already content-complete despite low ratio (Sanskrit terseness, not missing content). Validated (701 keys), rebuilt. **Remaining for next session:** the rest of the low-ratio cluster still needs individual verification — 2.48, 2.49, 2.50 (continuing the same karma-yoga/samatva passage as 2.47, likely same pattern), 2.11, 2.14–2.16, 2.39, 2.54, 2.61, then a full pass through the remaining ~60 verses not yet individually checked. Given chapter 2's size (72 verses, the longest chapter after 18), expect this to take multiple sessions — prioritize the low-ratio verses first per the pattern established in ch.1 (low ratio isn't always a real gap, but it's where real gaps concentrate). |
